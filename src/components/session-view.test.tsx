...
const mocks = vi.hoisted(() => ({
  ...
  buildQueuedPreviewState: vi.fn((_session?: unknown) => ({ mode: "queued", message: "queued preview" })),
  buildReadyPreviewState: vi.fn((_session?: unknown) => ({ mode: "ready", message: "ready preview", link: "https://preview.example" })),
  state: {
    autoMode: false,
...
function makeSession(overrides: Record<string, any> = {}): any {
  return {
    ...
    timeline: [
      { id: "remote-1", timestamp: 1, kind: "remote-turn", provider: "claude", text: "Are you ready for the next approval?" },
    ],
    ...
    managed: {
      ...
      responderProfile: { name: "claude-code", runtime: { model: "claude-opus-4-6" } },
      worktreeName: "main",
      awaitingInput: true,

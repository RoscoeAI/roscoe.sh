...
import { loadProjectContext, listProjectHistory, normalizeProjectContext, registerProject, saveProjectHistory, saveProjectContext, } from "./config.js";
...
function extractQuestionRecord(text) {
    ...
    try {
        ...
        return {
            ...
            options: parsed.options.filter((option) => typeof option === "string" && option.trim().length > 0),
...
function formatInterviewAnswersForPrompt(answers) {
    if (answers.length === 0)
        return "(none)";
...
function classifyInterviewTheme(source) {
    if (!source.trim())
        return null;
    if (/(project story|project-story|vision|intent)/.test(source))
        return "project-story";
    if (/(primary user|primary-users|users|operator|audience)/.test(source))
        return "primary-users";
    if (/(acceptance|proof|verification|acceptance check)/.test(source))
        return "acceptance-checks";
    if (/(success signal|success|outcome metric)/.test(source))
        return "success-signals";
    if (/(delivery pillar|delivery-pillar|frontend|front end|backend|back end|unit\/component|unit component|component tests|e2e|end-to-end|end to end)/.test(source))
        return "delivery-pillars";
    if (/(coverage mechanism|coverage command|coverage percent|measurable coverage|coverage stack|coverage tool|istanbul|nyc|vitest coverage|jest coverage|c8|coverage.py|go test cover|jacoco|lcov)/.test(source))
        return "coverage-mechanism";
    if (/(non-goal|non goals|non-goals|scope boundary|scope creep|out of scope)/.test(source))
        return "non-goals";
    if (/(constraint|constraints|limit|dependency)/.test(source))
        return "constraints";
    if (/(autonomy|escalation|approval|ask first)/.test(source))
        return "autonomy-rules";
    if (/(quality|testing|review|polish|bar)/.test(source))
        return "quality-bar";
    if (/(risk|danger|avoid|boundary)/.test(source))
        return "risk-boundaries";
    if (/(ui|ux|visual|design|frontend)/.test(source))
        return "ui-direction";
    if (/(definition of done|definition-of-done|completion|end state|\bdone\b)/.test(source))
        return "definition-of-done";
    return null;
}
function normalizeInterviewTheme(theme, question = "") {
...
function projectNeedsUiDirection(brief) {
    const text = [
        brief.name,
        brief.notes,
        ...brief.goals,
...
function getIntentAuditCorpus(brief) {
...
function hasFourPillarCompletionRule(brief) {
...
function hasCoverageStandard(brief) {
    const corpus = getIntentAuditCorpus(brief);
...
function hasMeasurableCoverageMechanism(brief) {
    const corpus = [
...
export class Onboarder extends EventEmitter {
    ...
    seedContext;
    seedHistory;
    ignoredSuccessfulExits = 0;
    constructor(projectDir, debug = false, profile = applyRuntimeSettings({
        name: "orchestrator",
        command: "claude",
        args: [],
        protocol: "claude",
    }, getDefaultOnboardingRuntime("claude")), projectRuntimeDefaults, options = {}) {
        super();
    ...
    start() {
        ...
        this.rawTranscript = "";
        this.session = new SessionMonitor(`orchestrator-${Date.now()}`, this.profile, dir);
        this.wireEvents();
    ...
    wireEvents() {
        ...
        this.session.on("thinking", (chunk) => {
            this.rawTranscript += `\n[thinking] ${chunk}`;
            this.emit("thinking", chunk);
        });
        this.session.on("tool-activity", (toolName) => {
            this.rawTranscript += `\n[tool] ${toolName}`;
            this.emit("tool-activity", toolName);
        });
        this.session.on("exit", (code) => {
     ...
     */
    sendInput(text, question, answerMeta) {
        if (!this.session)
            return;
        ...
        this.rawTranscript += `\n[user] ${text}\n`;
        const prompt = this.buildFollowUpPrompt(question?.question ?? "Unknown question", text);
        this.session.sendFollowUp(prompt);
    }
    updateRuntime(profile, projectRuntimeDefaults) {
        this.profile = profile;
    ...
    checkForProjectBrief() {
        const jsonMatch = this.outputBuffer.match(/---BRIEF---\s*\n?([\s\S]*?)\n?---END_BRIEF---/);
        if (jsonMatch) {
            try {
    ...
    auditInterviewReadiness(brief) {
        const coveredThemes = new Set(this.interviewAnswers
        ...
        const missingFields = [];
        if (!brief.intentBrief?.projectStory)
            missingFields.push("project story");
        if (!brief.intentBrief?.primaryUsers?.length)
            missingFields.push("primary users");
        if (!brief.intentBrief?.definitionOfDone?.length)
            missingFields.push("definition of done");
        if (!brief.intentBrief?.acceptanceChecks?.length)
            missingFields.push("acceptance checks");
        if (!brief.intentBrief?.successSignals?.length)
            missingFields.push("success signals");
        if (!brief.intentBrief?.deliveryPillars?.frontend?.length)
            missingFields.push("delivery pillars: frontend outcome");
        if (!brief.intentBrief?.deliveryPillars?.backend?.length)
            missingFields.push("delivery pillars: backend outcome");
        if (!brief.intentBrief?.deliveryPillars?.unitComponentTests?.length)
            missingFields.push("delivery pillars: unit/component test proof");
        if (!brief.intentBrief?.deliveryPillars?.e2eTests?.length)
            missingFields.push("delivery pillars: e2e test proof");
        if (!brief.intentBrief?.coverageMechanism?.length)
            missingFields.push("coverage mechanism with measurable percent");
        if (!brief.intentBrief?.nonGoals?.length)
            missingFields.push("non goals");
        if (!brief.intentBrief?.constraints?.length)
            missingFields.push("constraints");
        if (!brief.intentBrief?.autonomyRules?.length)
            missingFields.push("autonomy rules");
        if (!brief.intentBrief?.qualityBar?.length)
            missingFields.push("quality bar");
        if (!brief.intentBrief?.riskBoundaries?.length)
            missingFields.push("risk boundaries");

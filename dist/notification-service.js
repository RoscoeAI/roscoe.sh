...
function estimateProgress(summary, transcript) {
...
function buildNotificationBody(session, summary, estimate, urls) {
...
function normalizePublicWebhookUrl(publicValue, path) {
...
function readWebhookConfig() {
    ...
    const requestedMode = requestedModeRaw === "poll" || requestedModeRaw === "webhook" || requestedModeRaw === "auto"
        ? requestedModeRaw
        : "auto";
...
function parseTwilioPayload(text) {
    if (!text.trim())
        return null;
    try {
...
function parseTwilioListPayload(text) {
    if (!text.trim())
        return null;
    try {
        return JSON.parse(text);
...
export class NotificationService {
    ...
    seenInboundSids = new Set();
    webhookReplies = [];
    webhookServer = null;
    webhookStartError = null;
    webhookListeningPort = null;
    hasPendingQuestions() {
    ...
    async ensureInboundTransportReady() {
        const config = readWebhookConfig();
        if (config.effectiveMode !== "webhook")
            return;
    ...
    async sendTestMessage() {
        ...
        return this.sendSms(phoneNumber, "Roscoe wire is live. You’ll get milestone texts with progress estimates and evidence links.", { pollForFinalStatus: true });
    }
    async maybeSendProgressUpdate(session, summary) {
        const settings = loadRoscoeSettings();
        const phoneNumber = cleanPhoneNumber(settings.notifications.phoneNumber);
        ...
        await this.sendSms(phoneNumber, buildNotificationBody(session, summary, estimate, urls));
    ...
    async sendQuestion(session, question) {
        const settings = loadRoscoeSettings();
        const phoneNumber = cleanPhoneNumber(settings.notifications.phoneNumber);
        ...
        const result = await this.sendSms(phoneNumber, renderedQuestion, { pollForFinalStatus: false });
    ...
    async pollIncomingReplies() {
        const settings = loadRoscoeSettings();
        const phoneNumber = cleanPhoneNumber(settings.notifications.phoneNumber);
        const twilio = readTwilioConfig();
        if (!phoneNumber || !twilio)
            return [];
        const query = new URLSearchParams();
        query.set("From", phoneNumber);
        query.set("PageSize", "20");
        const auth = Buffer.from(`${twilio.accountSid}:${twilio.authToken}`).toString("base64");
        const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilio.accountSid}/Messages.json?${query.toString()}`, {
            method: "GET",
            headers: {
    ...
    async sendSms(to, body, options = {}) {
        ...
        if (twilio.fromNumber)
            form.set("From", twilio.fromNumber);
        if (twilio.messagingServiceSid)
            form.set("MessagingServiceSid", twilio.messagingServiceSid);
        const auth = Buffer.from(`${twilio.accountSid}:${twilio.authToken}`).toString("base64");
    ...
    async pollForFinalStatus(twilio, sid) {
    ...
    async fetchMessage(twilio, sid) {
        const auth = Buffer.from(`${twilio.accountSid}:${twilio.authToken}`).toString("base64");
        const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilio.accountSid}/Messages/${sid}.json`, {
            method: "GET",
            headers: {
                Authorization: `Basic ${auth}`,
            },
    ...
    async handleWebhookRequest(req, res, twilio, config) {

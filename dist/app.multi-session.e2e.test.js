...
describe.sequential("multi-session Ink e2e", () => {
    ...
    it("drives four concurrent conversations through errors, low confidence, and human intervention until all apps are working", async () => {
        ...
        vi.spyOn(config, "loadProfile").mockImplementation((name) => {
            if (name === "claude")
        ...
        const app = render(_jsx(App, { initialScreen: "session-view", startSpecs: startSpecs, initialAutoMode: true }));
        try {
            await waitForFrame(app.lastFrame, (frame) => ["pulse", "orbit", "atlas", "relay"].every((label) => frame.includes(label)) &&
            ...
            await manualInput(app, "atlas", "Decide whether history should stay linear", "Implement branch-aware version history with restore previews and keep backlink updates rename-safe.");
            await approveSuggestion(app, env.logPath, "pulse", "Finish collaborative filters before polishing");
            await manualInput(app, "orbit", "Tighten role filters", "Lock down role filters for admin and manager saved views, then finish anomaly drill-down and export parity.");
            await retryError(app, "relay", "Mock schema resolver crashed");
            await approveSuggestion(app, env.logPath, "relay", "Add secrets masking");
            await approveSuggestion(app, env.logPath, "orbit", "Run a production sanity pass");
            await waitForInvocationCount(env.logPath, 46, 60000);
            await assertSessionSuggestion(app, "pulse", "Ask for a final demo checklist and release-note pass.");
            await assertSessionSuggestion(app, "orbit", "Ask for release notes and an operator handoff checklist.");
            await assertSessionSuggestion(app, "atlas", "Ask whether to add onboarding docs for power users.");
            await assertSessionSuggestion(app, "relay", "Ask for a launch checklist and docs review.");
...
function createProject(root, projectsDir, label, appName, provider, sessionId) {
    const directory = join(projectsDir, label);
    mkdirSync(join(directory, ".roscoe"), { recursive: true });
    writeFileSync(join(directory, ".roscoe", "project.json"), JSON.stringify({
        name: label,
        directory,
...
function buildScenario(projects) {
...
function initialWorkerCall(project, lines, delayMs) {
    return {
...
function followUpWorkerCall(project, promptIncludes, lines, delayMs) {
    return {
        provider: project.provider,
        promptIncludes,
        promptExcludesAll: [
...
function suggestionCall(project, workerSignature, message, confidence, reasoning) {
    return {
        provider: project.provider,
        promptIncludesAll: [
            "Respond in this EXACT JSON format",
...
function errorSuggestionCall(project, workerSignature, stderr) {
    return {
        provider: project.provider,
        promptIncludesAll: [
            "Respond in this EXACT JSON format",
...
function summaryCall(project, workerSignature, text) {
    return {
        provider: project.provider,
        promptIncludesAll: [
            "Summarize what this AI coding session just accomplished",
...
async function retryError(app, label, errorText) {
    ...
    await pressUntil(app, "r", (frame) => isSessionActive(frame, label) && !frame.includes("error │") && !frame.includes(" error "));
}
async function approveSuggestion(app, logPath, label, text) {
...
async function manualInput(app, label, suggestionText, manualText) {
    await waitForReadySuggestion(app, label, suggestionText);
    await pressUntil(app, "m", (frame) => isSessionActive(frame, label) &&
...
async function assertSessionSuggestion(app, label, expectedText) {
    await waitForSessionText(app, label, expectedText, 15000);
}
async function waitForReadySuggestion(app, label, expectedText, timeoutMs = 20000) {
    const started = Date.now();
...
async function waitForSessionText(app, label, text, timeoutMs = 15000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
        await focusSession(app, label);
        const frame = app.lastFrame() || "";
...
async function focusSession(app, label) {
...
function isSessionActive(frame, label) {
    if (!frame)
        return false;
...
function directSessionKey(label) {
    switch (label) {
        case "pulse":
            return "1";
        case "orbit":
...
function createWrapper(root, provider, scenarioPath, statePath, logPath) {
...
function makeProfile(provider, command) {
    return {
        name: provider,
        command,
        args: [],
...
async function waitForInvocationCount(logPath, expected, timeoutMs = 15000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
        if (readInvocationLog(logPath).length >= expected)
            return;
        await delay(25);
    }
    throw new Error(`Timed out waiting for ${expected} invocations.\nCurrent log:\n${readFileSync(logPath, "utf-8")}`);
}
async function waitForFrame(lastFrame, predicate, timeoutMs = 15000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
...
async function pressUntil(app, key, predicate, timeoutMs = 3000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
        app.stdin.write(key);

...
function createSmsEntry(prefix, sessionId, entry) {
    smsEntryCounter += 1;
    return {
...
function deriveSmsQuestion(session) {
...
export function SessionView({ startSpecs, startRuntimeOverrides }) {
    ...
    useEffect(() => {
        if (!startSpecs || startSpecs.length === 0)
            return;
    ...
    const applyRuntimeEdit = (draft) => {
        ...
        for (const session of state.sessions.values()) {
            if (session.managed.projectDir !== activeSession.managed.projectDir)
                continue;
            if (detectProtocol(session.managed.profile) !== protocol)
                continue;
            ...
            const nextContext = normalizeProjectContext({
                ...
                runtimeDefaults: {
                    ...
                    workerByProtocol: {
                        ...
                        [protocol]: mergeRuntimeSettings(projectContext.runtimeDefaults?.workerByProtocol?.[protocol], runtimePatch),
                    },
                },
            });
    ...
    useInput((input, key) => {
        ...
        const approveSuggestion = async () => {
            if (!activeSession || activeSession.suggestion.kind !== "ready")
                return;
            await service.executeSuggestion(activeSession.managed, activeSession.suggestion.result);
        ...
        const retrySuggestion = async () => {
            if (!activeSession || activeSession.suggestion.kind !== "error")
                return;
    ...
    if (state.sessions.size === 0 && startSpecs && startSpecs.length > 0) {
        return (_jsx(Box, { padding: 1, children: _jsx(Text, { color: "yellow", children: "All sessions have ended. Press Ctrl+C to exit." }) }));
    }
    ...
    const sessionLabel = activeSession
        ...
        : undefined;
    return (_jsxs(Box, { flexDirection: "column", width: columns, height: rows, overflow: "hidden", children: [_jsxs(Box, { flexGrow: 1, children: [_jsx(SessionList, { sessions: state.sessions, activeSessionId: state.activeSessionId, width: railWidth }), _jsx(SessionOutput, { session: activeSession ?? null, sessionLabel: sessionLabel })] }), activeSession && runtimeEditorOpen && (_jsx(RuntimeEditorPanel, { protocol: detectProtocol(activeSession.managed.profile), runtime: activeSession.managed.profile.runtime, scopeLabel: "Changes apply to every Guild lane on this project/provider and take effect on the next turn.", onApply: applyRuntimeEdit })), activeSession && (_jsx(SuggestionBar, { phase: activeSession.suggestion, toolActivity: activeSession.currentToolUse, onSubmitEdit: (text) => {
                ...
                } })), _jsx(StatusBar, { projectName: currentProject, worktreeName: currentWorktree, autoMode: state.autoMode, sessionCount: state.sessions.size, viewMode: activeSession?.viewMode ?? "transcript", followLive: activeSession?.followLive ?? true, runtimeEditorOpen: runtimeEditorOpen })] }));
}

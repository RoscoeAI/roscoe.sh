...
import { listProfiles, listAuthProfiles, listRegisteredProjects, } from "./config.js";
...
function cleanRuntimeSettings(settings) {
    const entries = Object.entries(settings).filter(([, value]) => value !== undefined && value !== false && value !== "");
    if (entries.length === 0)
        return undefined;
    return Object.fromEntries(entries);
}
function buildProviderRuntimeOverrides(options) {
    const hasClaudeOverride = typeof options.claudeModel === "string" ||
        typeof options.claudeEffort === "string" ||
        typeof options.claudePermissionMode === "string" ||
        options.claudeDangerous === true;
    const hasCodexOverride = typeof options.codexModel === "string" ||
    ...
    if (hasCodexOverride) {
        const codex = cleanRuntimeSettings({
            ...
            approvalPolicy: options.codexApproval,
...
program
    .name("roscoe")
    .description("AI co-pilot that monitors LLM conversations, automates browser interactions, and orchestrates multi-agent workflows")
    .version("0.4.0")
...
program
    .command("start")
    .description("Start monitoring LLM sessions. Format: profile@dir:task or just profile")
    .argument("<specs...>", "Session specs: profile, profile@project-dir, or profile@project-dir:task-name")
...
program
    .command("onboard")
    .description("Onboard a project — analyze codebase, interview developer, generate system docs")
...
program
    ...
    try {
        ...
        console.error(`Failed to list worktrees: ${err instanceof Error ? err.message : err}`);
    }
});

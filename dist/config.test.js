...
import { loadProfile, listProfiles, loadProjectContext, registerProject, listRegisteredProjects, loadAuthProfile, listAuthProfiles, saveProjectHistory, listProjectHistory, loadRoscoeSettings, saveRoscoeSettings, } from "./config.js";
describe("config", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    describe("loadProfile", () => {
        it("reads and parses profile JSON", () => {
            vi.mocked(readFileSync).mockReturnValue(JSON.stringify({ name: "claude-code", command: "claude", args: ["--verbose"] }));
    ...
    describe("listProfiles", () => {
        it("returns profile names without .json extension", () => {
            vi.mocked(readdirSync).mockReturnValue(["claude-code.json", "codex.json", "readme.md"]);
    ...
    describe("loadAuthProfile", () => {
        it("reads and parses auth profile JSON", () => {
            vi.mocked(readFileSync).mockReturnValue(JSON.stringify({ name: "github", url: "https://github.com", steps: [] }));
    ...
    describe("listAuthProfiles", () => {
        ...
        it("filters for .json files", () => {
            vi.mocked(existsSync).mockReturnValue(true);
            vi.mocked(readdirSync).mockReturnValue(["github.json", "notes.txt"]);
        ...
        it("falls back to legacy .llm-responder memory when .roscoe is missing", () => {
            vi.mocked(existsSync).mockImplementation((path) => {
                ...
                if (filePath.includes("/tmp/project/.roscoe/project.json"))
                    return false;
                if (filePath.includes("/tmp/project/.llm-responder/project.json"))
                    return true;
                return false;
            });
    ...
    describe("registerProject", () => {
        it("adds new project to registry", () => {
            ...
            expect(writeFileSync).toHaveBeenCalledWith(expect.any(String), expect.stringContaining("myproj"));
        });
    ...
    describe("listRegisteredProjects", () => {
        ...
        it("filters ephemeral e2e projects and sorts by lastActive", () => {
            ...
            expect(writeFileSync).toHaveBeenCalledWith(expect.any(String), expect.not.stringContaining("roscoe-onboard-e2e"));
        });
    });
    describe("project history", () => {
        it("writes timestamped history files under the project history directory", () => {
            ...
            expect(writeFileSync).toHaveBeenCalledWith(expect.stringContaining("/tmp/proj/.roscoe/history/2026-03-25T12-00-00-onboard.json"), expect.stringContaining("\"rawTranscript\": \"raw turn transcript\""));
        });
        it("loads history records from disk", () => {
            vi.mocked(readdirSync).mockReturnValue(["2026-03-25T12-00-00-onboard.json"]);
            vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
                id: "2026-03-25T12-00-00-onboard",
                mode: "onboard",
                createdAt: "2026-03-25T12:00:00.000Z",
    ...
    describe("roscoe settings", () => {
        ...
        it("falls back to legacy settings path", () => {
            vi.mocked(existsSync).mockImplementation((path) => {
                const filePath = String(path);
                if (filePath.endsWith("/.roscoe/settings.json"))
                    return false;
                if (filePath.endsWith("/.llm-responder/settings.json"))
                    return true;
                return false;
            });
            vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
        ...
        it("writes settings under .roscoe", () => {
            ...
            expect(writeFileSync).toHaveBeenCalledWith(expect.stringContaining("/.roscoe/settings.json"), expect.stringContaining("+15551234567"));
        });
    });
});

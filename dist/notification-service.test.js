...
describe("NotificationService", () => {
    ...
    it("estimates higher completion when tests and deploy evidence exist", () => {
        expect(estimateProgress("Preview is deployed and Playwright coverage is green.", "assistant: Tests passed at https://preview.example.com").percent).toBeGreaterThanOrEqual(88);
    });
    ...
    it("sends a milestone text with summary and evidence", async () => {
        const service = new NotificationService();
        const sent = await service.maybeSendProgressUpdate(makeManagedSession(), "Playwright and Vitest are green and the preview is deployed.");
    ...
    it("deduplicates repeated progress updates", async () => {
        const service = new NotificationService();
        await service.maybeSendProgressUpdate(makeManagedSession(), "Playwright and Vitest are green and the preview is deployed.");
        const second = await service.maybeSendProgressUpdate(makeManagedSession(), "Playwright and Vitest are green and the preview is deployed.");
    ...
    it("reports queued vs delivered state for a test SMS", async () => {
        ...
        expect(fetchMock).toHaveBeenNthCalledWith(2, "https://api.twilio.com/2010-04-01/Accounts/AC123/Messages/SM123.json", expect.objectContaining({ method: "GET" }));
    });

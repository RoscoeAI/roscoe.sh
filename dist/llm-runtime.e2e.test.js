...
describe.sequential("provider e2e", () => {
    ...
    it("monitors a Claude-style session across resumed turns", async () => {
        ...
        try {
            const monitor = new SessionMonitor("worker-claude", makeProfile("claude", env.claudeCommand), env.projectDir);
            const textChunks = [];
    ...
    it("monitors a Codex-style session across resumed turns", async () => {
        ...
        try {
            const monitor = new SessionMonitor("worker-codex", makeProfile("codex", env.codexCommand), env.projectDir);
            const tools = [];
            const textChunks = [];
            monitor.on("tool-activity", (toolName) => {
                tools.push(toolName);
    ...
    it.each([
        ...
        try {
            ...
            const result = await generator.generateSuggestion("User: hello\n\nLLM: hi", provider, {
                profile,
    ...
    it.each(["claude", "codex"])("onboards a project with %s", async (provider) => {
...
function buildOnboardingScenario(provider) {

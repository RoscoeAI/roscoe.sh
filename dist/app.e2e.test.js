...
describe.sequential("Ink app e2e", () => {
    ...
    it.each(["claude", "codex"])("runs the full mocked session loop with %s", async (provider) => {
        ...
        const app = render(_jsx(App, { initialScreen: "session-view", startSpecs: [`${provider}@${env.projectDir}`], startRuntimeOverrides: {
        ...
        try {
            await waitForFrame(app.lastFrame, (frame) => frame.includes("Prompt:") &&
            ...
            expect(workerCall.args).toEqual(expect.arrayContaining(provider === "claude"
...
async function waitForFrame(lastFrame, predicate, timeoutMs = 5000) {
...
async function waitForInvocationCount(logPath, expected, timeoutMs = 5000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {

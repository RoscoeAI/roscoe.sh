...
import { chmodSync, existsSync, mkdirSync, readdirSync, mkdtempSync, readFileSync, writeFileSync, } from "fs";
...
describe.sequential("onboarder interview e2e", () => {
    ...
    it.each(["claude", "codex"])("requires a comprehensive Roscoe intake before onboarding completes with %s", async (provider) => {
        const env = createMockEnv(buildInterviewScenario(provider));
        const previousHome = process.env.HOME;
        process.env.HOME = env.projectDir;
        try {
            const profile = makeProfile(provider, provider === "claude" ? env.claudeCommand : env.codexCommand);
            ...
            currentOutput = "";
            turnDone = once(onboarder, "turn-complete");
            onboarder.sendInput("Frontend outcomes | Backend outcomes | Vitest | Playwright\n\nAll four pillars must move together before Roscoe can claim completion.", {
    ...
    it.each(["claude", "codex"])("refines a saved project brief with %s without rerunning full onboarding", async (provider) => {
        ...
        writeFileSync(join(projectDir, ".roscoe", "project.json"), JSON.stringify({
            name: "Refine Project",
        ...
        writeFileSync(join(projectDir, ".roscoe", "history", "legacy.json"), JSON.stringify({
            id: "legacy",
            mode: "onboard",
        ...
        try {
            const profile = makeProfile(provider, provider === "claude" ? env.claudeCommand : env.codexCommand);
            const onboarder = new Onboarder(projectDir, false, profile, undefined, {
                mode: "refine",
...
function buildInterviewScenario(provider) {
...
function buildRefineScenario(provider) {

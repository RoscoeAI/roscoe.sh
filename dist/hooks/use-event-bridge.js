...
export function buildInitialPrompt(managed, context) {
...
export function createPartialDispatcher(dispatch, id) {
    let lastTime = 0;
    let lastLen = 0;
    return (partial) => {
        const now = Date.now();
...
function createEntryId(prefix, sessionId) {
...
export function useEventBridge(sessions, dispatch, service, autoMode) {
    const autoModeRef = useRef(autoMode);
    autoModeRef.current = autoMode;
    const wiredRef = useRef(new Set());
    useEffect(() => {
        for (const [id, session] of sessions) {
            if (wiredRef.current.has(id))
                continue;
            ...
            const onExit = (code) => {
                if (managed._paused)
                    return; // intentional pause — don't remove
                if (code !== 0) {
                    dispatch({
                        type: "APPEND_TIMELINE_ENTRY",
                        id,

...
describe("debug-log", () => {
    ...
    it("enableDebug creates log directory and writes session header", async () => {
        ...
        expect(appendFileSync).toHaveBeenCalledWith(expect.stringContaining("debug.log"), expect.stringContaining("--- session"));
    });
    ...
    it("dbg writes formatted log line when enabled", async () => {
        ...
        expect(appendFileSync).toHaveBeenCalledWith(expect.any(String), expect.stringMatching(/\[\d{2}:\d{2}:\d{2}\.\d{3}\] \[proc\] spawned claude\n/));
    });
    it("dbg serializes non-string args as JSON", async () => {
        ...
        vi.mocked(appendFileSync).mockClear();
        dbg("test", { key: "value" });
        expect(appendFileSync).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('{"key":"value"}'));
    });
});

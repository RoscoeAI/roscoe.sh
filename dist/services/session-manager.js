...
import { detectProtocol, startOneShotRun, summarizeRuntime } from "../llm-runtime.js";
import { NotificationService } from "../notification-service.js";
import { applyRuntimeSettings, getLockedProjectProvider, getRuntimeTuningMode, getWorkerProfileForProject, mergeRuntimeSettings, recommendWorkerRuntime, } from "../runtime-defaults.js";
export class SessionManagerService {
    ...
    startSession(opts) {
        ...
        const monitor = new SessionMonitor(id, profile, worktreePath);
    ...
    async generateSuggestion(managed, onPartial) {
        ...
        return this.generator.generateSuggestion(context, managed.profileName, sessionInfo, onPartial, (trace) => {
    ...
    async executeSuggestion(managed, result) {
    ...
    async maybeNotifyProgress(managed, summary) {
        try {
    ...
    prepareWorkerTurn(managed, upcomingPrompt = "") {
                ...
                switch (action.type) {
                    ...
                    case "navigate":
                        if (action.params.url)
                            await this.browserAgent.open(action.params.url);
                        break;
                    case "snapshot":
                    ...
                    case "interact":
                        if (action.params.action && action.params.ref) {
                            await this.browserAgent.interact(action.params.action, action.params.ref, action.params.value);
                        }
                        break;
                }
            }
    ...
    async executeOrchestratorActions(actions) {
        if (!this.orchestrator)
            return;
        for (const action of actions) {
            try {
